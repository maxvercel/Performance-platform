...
export default function ExerciseLogger({ programExercise, workoutLogId }: Props) {
  ...
  return (
    <div className={`rounded-2xl overflow-hidden border transition-all duration-300 ${
      ...
      {/* Illustratie banner — alleen zichtbaar als expanded */}
      {expanded && (
      ...
      >
        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-black flex-shrink-0 transition ${
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3 border-t border-zinc-800 pt-3">
          ...
          {/* Sets */}
          {sets.map((s, i) => (
            <div key={i} className={`grid grid-cols-[32px_1fr_1fr_44px] gap-2 items-center transition-opacity ${
              s.done ? 'opacity-40' : ''
            }`}>
          ...
          <button
            onClick={addSet}
            className="w-full py-2 text-zinc-600 text-xs flex items-center
                       justify-center gap-1 hover:text-zinc-400 transition">
            + Extra set toevoegen
          </button>
        </div>
      )}
